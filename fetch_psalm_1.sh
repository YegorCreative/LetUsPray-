...
echo "Fetching Psalm 1 verses from API Bible (NASB1995)..."
echo "=========================================="
echo ""

# Create JSON array
echo "[" > psalm_1_verses.json

for verse in {1..6}; do
    echo "Fetching Psalm 1:$verse..."
    
    VERSE_ID="PSA.1.$verse"
    ...
    if [ $? -eq 0 ]; then
        ...
        cat >> psalm_1_verses.json << EOF
  {
    "verse": $verse,
    "reference": "Psalm 1:$verse",
    "text": $escaped_text
  }
EOF
    else
        echo "✗ Error fetching Psalm 1:$verse"
    fi
done
...
echo ""
echo "✅ Saved verses to psalm_1_verses.json"
echo ""
echo "=========================================="
echo "PSALM 1 VERSES (NASB1995)"
echo "=========================================="
cat psalm_1_verses.json | python3 -m json.tool
