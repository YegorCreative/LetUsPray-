...
# Fetch Psalm 122 verses
verses_data = []

for verse_num in range(1, 10):  # Verses 1-9
    verse_id = f"PSA.122.{verse_num}"
    ...
    print(f"Fetching Psalm 122:{verse_num}...")
    ...
    if response.status_code == 200:
        ...
        verses_data.append({
            'verse': verse_num,
            'reference': f"Psalm 122:{verse_num}",
            'text': verse_text
        })
        print(f"✓ Psalm 122:{verse_num}")
    else:
        print(f"✗ Error fetching Psalm 122:{verse_num}: {response.status_code}")
        print(f"Response: {response.text}")

# Save to JSON in proper location
output_dir = 'LetUsPray/LetUsPray/Resources/Bible/Psalms'
os.makedirs(output_dir, exist_ok=True)
output_file = os.path.join(output_dir, 'psalm_122_verses.json')
...
print("\n" + "="*80)
print("PSALM 122 VERSES (NASB1995)")
print("="*80)
