...
print("Fetching Psalm 11 verses from API Bible (NASB1995)...")
print("=" * 50)

for verse_num in range(1, 8):  # Verses 1-7
    verse_id = f"PSA.11.{verse_num}"
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            ...
            verses_data.append({
                'verse': verse_num,
                'reference': f"Psalm 11:{verse_num}",
                'text': verse_text
            })
            print(f"✓ Psalm 11:{verse_num}")
    except Exception as e:
        print(f"✗ Error fetching Psalm 11:{verse_num}: {e}", file=sys.stderr)

# Save to JSON
with open('psalm_11_verses.json', 'w', encoding='utf-8') as f:
    json.dump(verses_data, f, indent=2, ensure_ascii=False)

print(f"\n✅ Saved {len(verses_data)} verses to psalm_11_verses.json")
print("\n" + "=" * 50)
print("PSALM 11 VERSES (NASB1995)")
print("=" * 50)
